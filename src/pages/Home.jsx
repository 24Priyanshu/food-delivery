  ...
  return (
    <div className='bg-slate-200 w-full min-h-screen'>
      ...
      {!input ? <div className='flex flex-wrap justify-center items-center gap-5 w-full '>
        {Categories.map((item) => {
            return <div className='w-35 h-37.5 bg-white flex flex-col items-center gap-5 p-5 justify-start text-[20px]
      ...
      <div className={`w-full md:w-[40vw] flex flex-col items-center h-full fixed top-0 right-0 bg-white shadow-xl p-6
        transition-all duration-500 overflow-auto
        ${showCart ? "translate-x-0" : "translate-x-full"}`}>
        <header className='w-full flex justify-between items-center '>
          <span className='text-green-400 text-[18px] font-semibold'>Order Items</span>
          <RxCross2 className='text-green-400 w-7.5 h-5 text-[18px] font-semibold cursor-pointer hover:text-gray-600'
        ...
        <div className='w-full mt-9 flex flex-col gap-8'>
          {items.map((item)=>(
            <Card2 name={item.name} price={item.price} id={item.id} qty={item.qty} image={item.image}/>
          ))}
        </div>
        ...
        <button className='w-[80%] p-3 bg-green-500 rounded-lg text-white hover:bg-green-400 transition-all'
          onClick={() => {
